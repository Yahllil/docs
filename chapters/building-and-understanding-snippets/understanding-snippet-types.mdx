...
| **Primary View(s)** | This snippet type utilizes only the **Detail View**, showing all relevant fields for that singular record.                                                                        | This type includes two interconnected views:<br/><br/>• **List View:** The initial display showing a summarized list or overview of multiple records.<br/><br/>• **Detail View:** When a user clicks on a specific item in the List View, they navigate to a detailed view of that individual record. |
| **Preview**         | ![Single Item Snippet Preview](placeholder-single-item.png)<br/>*Screenshot of the Snippet Builder with "Single Item" selected and the phone preview showing a detailed view of one record.* | ![Multiple Items Snippet Preview](placeholder-multiple-items.png)<br/>*Screenshot of the Snippet Builder with "Multiple Items" selected and the phone preview showing a list view with multiple cards/items.*                                           |
...
[Gif showing the phone preview of a Multiple Items snippet, scrolling through the List View and then clicking an item to transition to its Detail View.]
...
<Accordion title="Navigating Back">
  ...
  *   In the **live, published snippet** (when accessed via its Shareable URL), users simply use their web browser's standard **back button** to return to the List View from the Detail View.
  ...
  [Screenshot highlighting the List View/Detail View toggle in the builder when in Multiple Items mode.]
</Accordion>
...
<Tab title="For Single Item Snippets">
  In Step 2 of the builder, you'll see a table preview of your Airtable data. You **select the specific record** you want this snippet to display by clicking the radio button next to its row. This selected record defines the core data for this *specific* Single Item snippet URL.

  [Screenshot highlighting the table preview in Step 2 and pointing to the radio button next to a record.]
...
<Tab title="For Multiple Items Snippets">
  ...
  [Screenshot highlighting the Filter and Permission controls in Step 2 for Multiple Items snippets.]
...
The power of user-based permission is that the *same base URL* (e.g., `` `https://app.trycrust.ai/snippets/<your_snippet_id>/view/multi` ``) can present *different* data to different users because the permission value is passed *dynamically* via the URL parameter (`` `?p=<permission_value>` ``).
