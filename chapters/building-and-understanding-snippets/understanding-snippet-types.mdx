...
The power of user-based permission is that the _same base URL_ (e.g., `` `https://app.trycrust.ai/<your_snippet_id>/list` ``) can present _different_ data to different users because the permission value is passed _dynamically_ via the URL parameter (`` `?p=<permission_value>` ``).
