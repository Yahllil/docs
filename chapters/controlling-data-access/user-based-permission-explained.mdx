...
<Tip icon="lock">
  **Personalized Data Access**  
  As discussed in [Understanding the Permission Mechanism](/chapters/controlling-data-access/understanding-the-permission-mechanism), you might not want every viewer to see the entire list.<br/>**User-based Permission** allows you to dynamically show only the records relevant to the *specific individual* viewing the snippet.
</Tip>
...
<Steps>
  <Step title="Configure the Permission Field">
    ...
    [Screenshot highlighting the "User-based permission" radio button selected and the dropdown menu for choosing the "Permission Field".]
  </Step>
  <Step title="Obtain the Viewer's Identifier (Permission Value)">
    ...
    <Tabs>
      <Tab title="Manually Submitting Input">
        ...
        [Screenshot of the "Access Required" modal asking for the specific identifier value.]
      </Tab>
      <Tab title="Pre-filled via URL Parameter (Direct URL / Magic Link)">
        ...
        [Screenshot highlighting a Shareable URL with the `?p=value` parameter appended.]
      </Tab>
    </Tabs>
  </Step>
...
<Steps>
    <Step title="Create Lookup Field (Airtable)">
        ...
        *   Source field: **Client** (Linked Record)
        *   Field to retrieve: **Email** (from Clients table)
        *   Name it: `Client Email Lookup`
        Click **Create field**. This field now shows the linked client's email on each campaign record.

        [Gif showing the process of adding a Lookup field in Airtable: clicking +, selecting Lookup, choosing the linked record field, choosing the Email field, naming it "Client Email Lookup", and saving.]
    </Step>
    <Step title="Configure Permission (Crust AI)">
        ...
        *   In the "Which field holds the user's identifier?" dropdown, select your new **"Client Email Lookup"** field.
...
<Steps>
    <Step title="Create Formula Field (Airtable - Clients Table)">
        ...
        *   Name it: `Client Record ID`. Save it.
        ...
        [Screenshot of adding a Formula field in Airtable, entering RECORD_ID(), and naming it "Client Record ID".]
    </Step>
    <Step title="Create Lookup Field (Airtable - Campaigns Table)">
        Go to your **Campaigns** table. Add a new **Lookup** field. Configure it:
        *   Source field: **Client** (Linked Record)
        *   Field to retrieve: **"Client Record ID"** (the formula field you just created in the Clients table).
        *   Name it: `Client Record ID Lookup`. Save it.

        [Screenshot of adding the second Lookup field in the Campaigns table, this time selecting the "Client Record ID" formula field from the Clients table.]
    </Step>
    <Step title="Configure Permission (Crust AI)">
        Go back to your Campaigns snippet in the Crust AI Builder (refresh if needed). Navigate to **Step 2 -> Permission**.
        *   Select **User-based permission**.
        *   Choose the **"Client Record ID Lookup"** field as the Permission Field.
        *   Click **Apply Permission**.
    </Step>
</Steps>

Now, the permission check relies on the much more secure Airtable Record ID. The "Permission Value" needed will be the client's Record ID (e.g., `recABCXYZ`). You would typically *only* share this via Direct URLs generated through Airtable automations or buttons, embedding the specific client's Record ID into the `?p=` parameter, making it function like a secure magic link.
...
[Gif showing a user entering an email/Record ID into the "View as" box, the previews filtering, and then clicking the "Copy Direct URL" icon.]
</Accordion>
